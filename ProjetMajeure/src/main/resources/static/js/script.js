...
mapboxgl.accessToken = 'pk.eyJ1IjoidHNpbGlhIiwiYSI6ImNsNDJ1ajRsMjB5aDUzb3FsYjBiZ3J4Y2YifQ.-PG2Q4r2pbwpZoGgq2GNvA';
...
async function retour_caserne(){
    var data_vehicle = await getapi('vehicle');
    var our_vehicle = data_vehicle.filter( element => element.facilityRefID == 662629);
...
async function place(param, data){    
    ...
    for (let r in data) {
        ...
        if (param == 'facility'){
            ...
            facility_marker.bindPopup('name: '+data[r].name + '<br> maxVehicleSpace: ' + data[r].maxVehicleSpace+ '<br> peopleCapacity: ' + data[r].peopleCapacity + '<br> ' + data[r].lat + '<br> ' + data[r].lon)
        ...
        if (param == 'vehicle'){
            ...
            if (data[r].facilityRefID == 662629){
...
async function setVehicle(id, lat_fin, lon_fin){
    ...
    for (let r in data_vehicle){
        if ((data_vehicle[r].id == id) && (data_vehicle[r].facilityRefID == 662629)){
...
async function getVehicle(){
    ...
    var data_vehicle = data.filter( element => element.facilityRefID == 662629);
...
async function getRoute(id_vehicle, id_fire) {
  // make a directions request using driving profile
    var data_vehicle = await getapi('vehicle');
    var data_fire = await getapi('facility');
  ...
  /*const query = await fetch(
    'https://api.mapbox.com/directions/v5/mapbox/driving/'+lat_deb+','+lon_deb+';'+lat_fin+','+lon_fin+'?steps=true&geometries=geojson&access_token=${mapboxgl.accessToken}',
    { method: 'GET' }
  );*/
    ...
    fetch('https://api.mapbox.com/directions/v5/mapbox/driving/'+lat_deb+','+lon_deb+';'+lat_fin+','+lon_fin+'?geometries=geojson&access_token=pk.eyJ1IjoidHNpbGlhIiwiYSI6ImNsNDJ1ajRsMjB5aDUzb3FsYjBiZ3J4Y2YifQ.-PG2Q4r2pbwpZoGgq2GNvA', requestOptions)
      .then(response => response.json())
      .then(response => callback(response))
      .catch(error => console.log('error', error));
 }

getRoute(662669, 662629);

function callback(data){
    var coords = data.routes[0].geometry.coordinates;
    console.log("coordinates: "+coords +" de type "+typeof(coords))
    send_itenerary(coords);
    
}

function send_itenerary(response){
    var myHeaders = new Headers();
    myHeaders.append("Content-Type", "application/json");
